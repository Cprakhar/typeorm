        ...
        it("should set the table database / schema", () =>
            Promise.all(
                dataSources.map(async (dataSource) => {
                    ...
                    expect(table!.name).to.equal("testDB.questions.question")
                }),
            ))
                ...
                it("should correctly create tables when custom table schema used in Entity decorator", () =>
                    Promise.all(
                        dataSources.map(async (dataSource) => {
                            ...
                            expect(table!.name).to.be.equal("userSchema.user")
                        }),
                    ))
                ...
                it("should correctly work with cross-schema queries", () =>
                    Promise.all(
                        dataSources.map(async (dataSource) => {
                            ...
                            expect(table!.name).to.be.equal("guest.category")
                        }),
                    ))
